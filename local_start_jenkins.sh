...
export JENKINS_HOME=~/final_project/webpage_ws/.jenkins
mkdir -p $JENKINS_HOME

# Download the Jenkins .war file, if not there already
cd ~/final_project/webpage_ws/.jenkins
JENKINS_FILE="/home/invokelee/final_project/webpage_ws/.jenkins/jenkins.war"
...
if [[ $string == *"jenkins.war"* ]]; then
    ...
    URL=`echo "https://localhost:8080/jenkins/"`
    echo ""
    ...
    STATE_FILE=~/final_project/webpage_ws/jenkins__pid__url.txt
    touch $STATE_FILE
